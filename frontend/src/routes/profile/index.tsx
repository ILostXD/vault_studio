								...
								<div className="space-y-4">
									...
									<div className="flex min-w-0 flex-col gap-3 border-b border-(--card-border) pb-4">
										<div className="min-w-0">
											...
											<p className="text-(--text-2) text-sm">
												Preferred relative key
											</p>
										</div>
										<fieldset
											disabled={!preferences || savingKeyPreference}
											aria-label="Automatic key detection"
											className="min-w-0 w-full disabled:opacity-50"
										>
											<ToggleGroup
												className="w-full"
												options={[
												...
												onValueChange={async (value) => {
													...
													try {
														await updatePreferences({
															key_mode_preference: value as KeyModePreference,
														});
													} catch {
								...
								<div className="bg-linear-to-b from-(--card-gradient-from) to-(--card-gradient-to) border border-(--card-border) rounded-3xl p-6">
									...
									<div className="space-y-4">
										<div className="flex items-center justify-between border-b border-(--card-border) pb-4">
											...
											<Button
												className="w-20 justify-center text-center bg-(--muted-2) rounded-[7px] px-4 py-1 text-(--text-0) text-sm font-medium h-auto hover:bg-(--muted-1)"
										...
										<div className="flex items-center justify-between">
											...
											<Button
												variant={"destructive"}
												className="w-20 justify-center text-center bg-[#381d1d] rounded-[7px] px-4 py-1 text-[#ff5656] text-sm font-medium h-auto hover:bg-[#6a2a2a] border-[#7f3434] border-[0.5px]"
